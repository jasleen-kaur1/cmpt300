...
// run_enzyme is a pointer to a void taking a single parameter data that is a pointer to a void
void *run_enzyme(void *data) {
	...
	*/
	...
	/* On return this function returns 0. Old type is not required, thus using NULL */
	pthread_setcanceltype(PTHREAD_CANCEL_ASYNCHRONOUS, NULL);
	// printf("thread_data->string[0] %c \n", thread_data->string[0]);
	
	// pthread_self() returns the particular thread id
		...
		if ((void*)status == PTHREAD_CANCELED) {
		    ...
		    printf("Thread %d did not return anything\n",i);
     		...
     		printf("Thread %d exited normally: ",i);// Don't change this line
      		thread_info_t *thread_info = (thread_info_t *)status;
      		int threadswapcount = thread_info -> swapcount; 
...
int smp2_main(int argc, char **argv) {
	...
	pthread_t sleeperid;
	// attr argument is NULL, meaning the thread is created with default attributes
