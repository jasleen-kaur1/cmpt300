...
{   
    ...
    // sigwait function suspends the execution of the calling thread until it receives a SIGUSR2 resume signal to be pending
	// wait for a signal SIGUSR2
	sigwait(&set_block, &sig);
...
void *start_worker(void *arg)
{
	...
	sigaddset(&set_block, SIGALRM);
	sigaddset(&set_block, SIGUSR2);
	pthread_sigmask(SIG_BLOCK, &set_block, NULL);
	 
	/* TODO: Unblock SIGUSR1 and SIGTERM. */
	sigemptyset(&set_unblock);

	sigaddset(&set_unblock, SIGUSR1);
	sigaddset(&set_unblock, SIGTERM);
	pthread_sigmask(SIG_UNBLOCK, &set_unblock, NULL);
