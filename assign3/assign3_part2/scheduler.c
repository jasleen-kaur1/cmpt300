...
// Made sigaction into global variables. Helps pass all tests everytime
struct sigaction action_timer;
struct sigaction action_cancel;
struct sigaction action_suspend;
...
void cancel_worker(thread_info_t *info)
{
	...
	// pthread_kill sends a signal SIGTERM to thread
...
void setup_sig_handlers() {
	...
	action_timer.sa_handler = timer_handler;

	// Means no flags are used to modify the behaviour of the signal
	action_timer.sa_flags = 0;

	// Initializes a set of signals (blocked signals in this case) to an empty set
	...
	// struct sigaction action_cancel;
	action_cancel.sa_handler = cancel_thread;
	action_cancel.sa_flags = 0;
	sigemptyset(&action_cancel.sa_mask);

	sigaction(SIGTERM, &action_cancel, NULL);

	/* Setup suspend handler for SIGUSR1 signal in workers */
	// struct sigaction action_suspend;
	action_suspend.sa_handler = suspend_thread;
	action_suspend.sa_flags = 0;
	sigemptyset(&action_suspend.sa_mask);

	sigaction(SIGUSR1, &action_suspend, NULL);
